...
type PaymentWayProps = {params: {ngoSlug: string}};

const PaymentWay: FunctionComponent<PaymentWayProps> = ({params}) => {
    const router = useRouter();
    const {setDonatorInfo} = useDonatorInfo();
    ...
    function onSubmit(values: TFormSchema): void {
        ...
        if (values.paymentWay === 'boleto') {
            return router.push(`/${params.ngoSlug}/confirmacao`);
        }

        router.push(`/${params.ngoSlug}/pagamento`);
    }
...
const formSchema = z.object({
    paymentWay: z.enum(['card', 'boleto'], {required_error: 'Selecione uma forma de pagamento'}),
});
